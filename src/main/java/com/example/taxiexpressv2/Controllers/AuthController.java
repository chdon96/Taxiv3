...
import com.example.taxiexpressv2.Services.AuthService;
import com.example.taxiexpressv2.dto.AuthRequest;
import com.example.taxiexpressv2.dto.UserCredentialResponse;
import com.example.taxiexpressv2.dto.UserResponse;
...
public class AuthController {
    ...
    public ResponseEntity<EmployeCredential> registerUser(@RequestBody EmployeCredential user) {
        try {
            EmployeCredential registeredUser = authService.registerUser(user);
    ...
    public ResponseEntity<UserCredentialResponse> getToken(@RequestBody AuthRequest authRequest) {
        ...
        if (authenticate.isAuthenticated()) {
            ...
            Optional<EmployeCredential> userOptional = authService.getUserByUsername(authRequest.getUsername());
            ...
            EmployeCredential safeUser = new EmployeCredential();
    ...
    @GetMapping("/byRole")
    public List<EmployeCredential> getAllUsersByRole(@RequestParam UserRole role) {
    ...
    public ResponseEntity<UserResponse> getUserById(@PathVariable Integer id) {
        Optional<EmployeCredential> userOptional = authService.getUserById(id);

        if (userOptional.isPresent()) {
            EmployeCredential user = userOptional.get();
